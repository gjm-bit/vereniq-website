...
import { ConsentRevokeControl } from "@/src/components/consent-revoke-control";

type Content = { title: string; intro: string; sections: { heading: string; body: string }[] };

const legal: Record<string, Content> = {
  privacy: { title: "Privacy", intro: "Meer Vereniging wordt ontwikkeld met privacy by design en de toepasselijke AVG-verplichtingen als uitgangspunt.", sections: [{ heading: "Zorgvuldig met gegevens", body: "We verwerken alleen gegevens die nodig zijn voor het doel waarvoor ze worden gebruikt. Rollen en rechten helpen om informatie bij de juiste mensen te houden." }, { heading: "Website-statistieken", body: "We houden anonieme, sessiegebonden bezoekstatistieken bij, zonder cookies. Als je daarvoor toestemming geeft, onthouden we op je apparaat ook dát je de website al eerder hebt bezocht - geen naam, geen profiel. Lees hier meer over op onze cookiepagina." }, { heading: "Juridische status", body: "De definitieve privacyverklaring wordt vóór commerciële ingebruikname juridisch gecontroleerd, versiegestuurd vastgesteld en hier gepubliceerd." }] },
  cookies: { title: "Cookies", intro: "We houden het gebruik van cookies beperkt en helder.", sections: [{ heading: "Wat we nu gebruiken", body: "De website gebruikt alleen noodzakelijke technische middelen om pagina’s veilig en bruikbaar te laten werken, en houdt anonieme, sessiegebonden bezoekstatistieken bij (zoals hoeveel pagina’s worden bekeken). Dit gebeurt zonder cookies en zonder dat we je herkennen tussen bezoeken door." }, { heading: "Herkenning van een eerder bezoek (met jouw toestemming)", body: "Als je toestemming geeft, onthouden we op dit apparaat alleen dát je de website al eerder hebt bezocht - geen naam, geen profiel, geen gegevens die naar jou herleidbaar zijn. Dit gebruiken we uitsluitend om te zien of bezoekers die via een AI-platform (zoals ChatGPT) binnenkomen nieuw zijn of terugkeren. Zonder toestemming werkt de website precies hetzelfde, alleen blijft dat onderscheid dan onbekend." }, { heading: "Als dit verandert", body: "Wanneer we ooit iets anders inzetten dan hierboven beschreven, informeren we je vooraf en vragen we toestemming waar dat nodig is." }] },
...
const pageMeta: Record<string, [string, string]> = {
  ...
  demo: ["Meer weten?", "Vertel iets over je vereniging. Dan kijken we samen of en hoe Meer Vereniging kan aansluiten."],
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string[] }> }) {
  ...
  if (cmsPage) return { title: cmsPage.seoTitle ?? cmsPage.title, description: cmsPage.seoDescription ?? undefined, robots: cmsPage.noindex ? { index: false, follow: false } : undefined, alternates: { canonical } };
...
function LegalPage({ content, extra }: { content: Content; extra?: ReactNode }) { return <section className="section"><div className="container" style={{ maxWidth: 900 }}>{content.sections.map((section) => <section key={section.heading} style={{ marginBottom: 36 }}><h2 style={{ fontSize: "clamp(1.7rem,3vw,2.4rem)" }}>{section.heading}</h2><p className="lede">{section.body}</p></section>)}{extra}<Link className="btn btn-secondary" href="/contact">Neem contact op</Link></div></section>; }
...
export default async function CommercialPage({ params }: { params: Promise<{ slug: string[] }> }) {
  ...
  return <PublicShell><section className={(slug === "prijzen" || slug === "over-ons" || legalContent) ? "hero" : "section section-mist"}><div className="container"><p className="eyebrow">Meer Vereniging</p><h1>{title}</h1><p className="lede">{intro}</p></div></section>{slug === "prijzen" ? <Pricing /> : slug === "over-ons" ? <About /> : slug === "contact" ? <Form demo={false} /> : slug === "demo" ? <Form demo /> : legalContent ? <LegalPage content={legalContent} extra={slug === "cookies" ? <ConsentRevokeControl /> : null} /> : null}</PublicShell>;
}
