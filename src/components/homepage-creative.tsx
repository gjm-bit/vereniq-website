...
export function HomepageCreative({ content: rawContent }: { content?: HomepageContent }) {
  ...
  return <div className="mv-homepage mv-compressed-home">
    ...
    <section className="mv-compressed-account" aria-labelledby="mv-account-title"><div className={`mv-page-width mv-account-grid${mediaUrl(content.account.media) ? "" : " mv-account-grid-solo"}`}><div><p className="mv-kicker">{content.account.eyebrow}</p><h2 id="mv-account-title">{content.account.title}<br /><em>{content.account.accentTitle}</em></h2><p className="mv-section-body">{content.account.body}</p></div>{mediaUrl(content.account.media) ? <ProductVisual variant="organization" label="Organisatie-wisselaar in Meer Vereniging" media={content.account.media} /> : null}</div></section>
